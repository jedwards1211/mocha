...
const createWatcher = (
  ...
  let watcher, globalSetupBarrier;
  /* istanbul ignore next */
  if (TEST_MODE) {
    const EventEmitter = require("node:events");
    watcher = new EventEmitter();
    let resolveGlobalSetup;
    globalSetupBarrier = new Promise(
      (resolve) => (resolveGlobalSetup = resolve)
    );
    process.on("message", (msg) => {
      debug("got message", msg);
      if (msg.watcher) watcher.emit(...msg.watcher);
      if (msg.resolveGlobalSetup) resolveGlobalSetup();
    });
  } else {
    ...
    globalSetupBarrier = Promise.resolve();
  }
  ...
  watcher.on("ready", () => {
    ...
    ready = true;
    /* istanbul ignore next */
    if (TEST_MODE) sendMessage({ received: ["ready"] });
  });

  watcher.on("all", function handleEvent(event, filePath, stat) {
    /* istanbul ignore next */
    if (TEST_MODE) sendMessage({ received: ["all", event, filePath] });
    if (!matcher.allow(filePath)) {
      debug("event doesn't match filter: %s, %s", event, filePath);
      return;
    }
    function doHandle(event, filePath, stat) {
      debug(
        'doHandle [event: %s] [filePath: %s] [stat.birthtime: %d] [stat.mtime: %d] [ready: %s] [globalFixtureContext: %s] [exiting: %s]',
        event,
        filePath,
        stat?.birthtime,
        stat?.mtime,
        ready,
        globalFixtureContext != null ? 'defined' : 'not defined',
        exiting
      );
      if (exiting) return;
      if (event === "unlink" || (stat ? Math.max(stat.mtime, stat.birthtime) > startTime : ready)) {
      ...
      if (!ready && !stat) {
        fs.stat(filePath, (err, stat) => {
          if (stat) doHandle(event, filePath, stat);
          else debug("failed to stat %s: %s", filePath, err.stack);
        });
      }
    }
    doHandle(event, filePath, stat);
  });

  /* istanbul ignore next */
  if (TEST_MODE) {
    sendMessage({ listening: true });
  }

  debug("triggering global setup");
  mocha
    .runGlobalSetup()
    .then((context) => globalSetupBarrier.then(() => context))
    .then((context) => {
